...
import { ProjectMetrics, SonarClient, SonarConfig } from "./Types";
...
export class SonarRepository {
  ...
  async projectMetrics(sonarConfig: SonarConfig): Promise<ProjectMetrics[]> {
    const searchHistoryUrl = `/api/measures/search_history?component=${
    ...
    //TODO pagination
    const measures = await this.sonarClient.getData(searchHistoryUrl);
